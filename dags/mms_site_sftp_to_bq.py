...
REMOTE_PATH = os.getenv("AIRFLOW_CONN_MMS_SITE_SFTP_PATH") 
# GCP config
service_account_info = json.loads(os.environ["GCP_SA_KEY"])

GCS_BUCKET = 'ur-pipeline-testing'
GCS_PREFIX = "raw/mms/play/MMS_Site"

BQ_DATASET_ID = "dbt_vfinta"
BQ_TABLE_ID = "source_mms_site"
...
def sftp_to_gcs(ds, **context):
    ...
    sftp = paramiko.SFTPClient.from_transport(
        transport, window_size=2**25, max_packet_size=32768
    )
    print("Connected to SFTP server, looking for files...")

    try:
        remote_filename = next(
            (f for f in sftp.listdir(REMOTE_PATH) if f.startswith(fetch_date)), None
        )
        if not remote_filename:
            raise FileNotFoundError(f"No SFTP file found for date {fetch_date}")
        else:
            print(f"Found SFTP file: {remote_filename}")

        gcs_blob_path = f"{GCS_PREFIX}/{remote_filename}"

        with sftp.open(os.path.join(REMOTE_PATH, remote_filename), "rb") as f:
            f.prefetch()
            data = f.read()  # fast read from prefetch buffer
            print(f"Read {remote_filename} from SFTP, size: {len(data)} bytes")
